...
*  v1.1  2025-07-15  Xiaopang   添加宏定义，增强可移植性
*
* 注意事项：
*  1. 使用前需确保HAL库初始化完成
...
#define OLED_GPIO_CLK_ENABLE()       do{ RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE); }while(0) /* 时钟使能宏 */
...
#define OLED_W_SCL(x)		do{ x ?  \
								GPIO_WriteBit(OLED_GPIO_PORT, OLED_SCL, (BitAction)(1)) : \
								GPIO_WriteBit(OLED_GPIO_PORT, OLED_SCL, (BitAction)(0)); \
							}while(0)
...
#define OLED_W_SDA(x)    	do{ x ?  \
								GPIO_WriteBit(OLED_GPIO_PORT, OLED_SDA, (BitAction)(1)) : \
								GPIO_WriteBit(OLED_GPIO_PORT, OLED_SDA, (BitAction)(0)); \
							}while(0)
